...
The initialisation of the GMM affects the convergence rate and possibly the resulting estimate. However, when only diagonal covariance matrices are used, the initialisation does not have a significant effect \cite{reynolds93}. The GMM was initialised with vector quantisation (VQ\abbrev{VQ}{vector quantisation}) where the variance and priors was estimated from the quantised vectors.
