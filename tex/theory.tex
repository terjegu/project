...
\begin{figure}[htbp]
  \centering
   \begin{tabular}[h]{c}
\xymatrix{ 
\mathbf{x} \ar[r] &*+<5mm>[F-,]{\txt{Buffer}} \ar[d]\ar[r] &*+<5mm>[F-,]{1-A(z)}\ar[rr]^{\mathbf{e}}& &*+<5mm>[F-,]{\frac{1}{1-\tilde{A}(z)}} \ar[r] & \mathbf{y}\\
...
&*+<5mm>[F-,]{\txt{DTW}} \ar @<3pt>[rr]^<(0.15){\tilde{\mathbf{x}}} \ar @<-3pt>[rr]_<(0.15){\mathbf{y}} & \ar[u] &*+<5mm>[F-,]{\txt{Train VC}}\\ % end line 2
...
The available data for the conversion function is time-aligned source and target vectors, Section~\ref{sec:dynamic_time_warping}, which are of the same length $n$ and the pre-trained gaussian mixture models. A conversion function suggested by Yannis Stylianou \etal \cite{stylianou95} is used in this paper:
\begin{defn}
